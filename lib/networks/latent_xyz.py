...
class Network(nn.Module):
    ...
    def forward(self, sp_input, grid_coords, viewdir, light_pts):
        ...
        xyz = feature[..., :3]
        xyz = spconv.SparseConvTensor(xyz, coord, out_sh, batch_size)

        xyz_features = self.xyz_net(xyz, grid_coords)

        net = self.actvn(self.fc_0(xyz_features))
        ...
        latent = self.latent(sp_input['i'] - 1)
